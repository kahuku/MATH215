...
Original file is located at
    https://colab.research.google.com/drive/1xeNgYsN0n9IQtm8mFpI3kjArl8G3Tzt5
...
vert_stretch = stretch(iden, 1, 2)
rotated = rotate(vert_stretch, -np.pi/4)
reflected = reflect(rotated, 2, -3)
comp_matrix = stretch(reflected, 1/2, 1)

# horizontal_stretch = stretch(iden,1/2,1)
# reflected = reflect(horizontal_stretch,2, -3)
# rotated = rotate(reflected,-np.pi/4)
# comp_matrix= stretch(rotated,1, 2)
...
def area(A): 
  ...
  return np.linalg.norm(v) * np.linalg.norm(w) * np.sin(theta)
...
#ellipse_area = np.linalg.solve(c, ellipse_matrix)

ellipse_area = np.absolute(np.linalg.det(ellipse_matrix)) * np.pi
