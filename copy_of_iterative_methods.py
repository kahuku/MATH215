...
Original file is located at
    https://colab.research.google.com/drive/1_d8Peicpai7Kt_Xr557FlX2FP_GscEc7
...
def gs1_error(n): 
    l = gs1_method(n)
    error = np.linalg.norm(np.array(l)-np.array([x_val, y_val]))
    return error
...
def gs2_error(n): 
    l = gs2_method(n)
    error = np.linalg.norm(np.array(l)-np.array([2, 1]))
    return error
...
def gs3_method(n): 
    ...
    z_n=0
    for i in range(n):
      l = gs3_iteration(x_n,y_n,z_n)
      x_n, y_n, z_n = l[0], l[1], l[2]
    return [x_n,y_n,z_n]
